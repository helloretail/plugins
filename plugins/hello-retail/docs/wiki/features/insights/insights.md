---
source: public-docs
verified: never
---

# Insights *(free)*

## What it is

Simple analytics layer that surfaces trends and key findings about the customer's store performance and the wider ecommerce market. Free with any paid Hello Retail plan.

## What it shows

- Store-level trends and performance signals.
- Cross-store / market-level patterns (where allowed by privacy).
- Key findings — e.g. categories trending up, products with rising velocity.

This is a lighter-weight analytics layer separate from per-feature analytics (Search Analytics, Pages Analytics, Recommendations Analytics, Triggered Emails Analytics, etc.) — Insights is more about "what's happening in your store overall".

## D&TS notes

- Insights pairs well with **Audience** in EBRs — together they give the customer a 360° picture without paying for an extra analytics tool.
- Per-feature analytics dashboards (Search, Pages, etc.) live inside each feature's own panel — don't conflate.

## Key support articles

- [Insights Overview](https://support.helloretail.com/insights/insights-overview/)

## Developer documentation

- No dedicated section in [developer.helloretail.com](https://developer.helloretail.com/) — this feature is configured via the dashboard.

## Sources

- [support.helloretail.com/insights](https://support.helloretail.com/insights/)

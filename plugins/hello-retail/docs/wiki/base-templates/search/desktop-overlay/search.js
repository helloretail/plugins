...
function activate() {
	...
	searcher.initial_render(function(template) {
		...
		if (close_on_backdrop_click) {
			...
			overlay.addEventListener("click", function(e) {
				...
				if (e.target.classList.contains("hr-results-container") ||
						e.target.classList.contains("hr-logo-container") ||
						e.target.classList.contains("hr-products-container") ||
						e.target.classList.contains("hr-content")) {
					close_overlay();
				}
			});
		}
	...
	function load_more_results(append, callback) {
		...
		searcher.yield_template(function(template, state) {
			...
			if (append) {
				overlay.querySelector(".hr-results .hr-products .hr-products-container")
						.appendChild(template(".hr-products-container .hr-search-overlay-product"));
			} else {
	...
	function open_overlay() {
		overlay.open();
		setTimeout(function() { focusElement(overlay_input_selector); }, 50);
	...
	function handle_live_update() {
		...
		const skipResultElement = document.querySelector("#hr-skip-search-result");
		...
		setTimeout(() => {
			...
			if (skipResultElement) {
				skipResultElement.textContent = headerElement.textContent;
			}
		}, 500);
	}
	...
	function handle_skip_content() {
		...
		skipToProducts.forEach((el) => {
			el.setAttribute("tabindex", enableButton ? "0" : "-1");
		});

		// handle click logic - use data attribute to avoid registering duplicate listeners
		document.querySelectorAll(".skip-content")?.forEach((link) => {
			if (link.dataset.skipListenerRegistered) return;
			link.dataset.skipListenerRegistered = "true";
		...
		// handle focus on input - only register once
		var hrSearch = document.querySelector("#hr-search");
		if (hrSearch && !hrSearch.dataset.focusListenerRegistered) {
			hrSearch.dataset.focusListenerRegistered = "true";
			hrSearch.addEventListener("focus", function() {
				setTimeout(() => {

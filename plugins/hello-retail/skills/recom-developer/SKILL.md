...
**The operator invokes only this skill — you pull in the tile skill yourself here; never ask them to run it separately.** Invoke **`tile-extractor`** (call the Skill tool with that name, or read `../tile-extractor/SKILL.md` and follow it) against the **same surveyed category page(s)** from Step 2 plus the `website-uuid` (so it fetches feed fields via `productData_get`) and the resolved `locale`. **Tell it `target surface = recom`** (injected into the live page; customer theme CSS applies) so it returns the `{{ TILE_BODY }}` Liquid **markup only, authors no CSS, and emits every fixed text as an `{% input %}` block**. It returns the copied customer tile with the product values bound by table, the state branches, the shop's own ATC form markup, and the sections you read by name: **FIDELITY** (every state PASS or fully classified before you drop the tile in — a *shell-side* item there becomes a restated theme rule or a NOTES line, a *markup* item goes back to the tile skill, never into your CSS), **PARENT HOOKS** (products-container hooks go on the `#hello-retail-{{ key }}` wrapper, cell-level hooks on the `.hr-product` cell — scope classes only, never width or layout classes), **TEXT INPUTS** (the `{% input %}` names with their native values; nothing to declare — they become dashboard fields — but they go into the hand-off so the operator fills each per domain), **BINDINGS** and **PARITY TABLE** (build record), **MISSING DATA** and **OPEN QUESTIONS** (carried forward; the sized-images and global-CSS questions go to the operator with the Step 6 diff). Hold that result; drop it into the slot in Step 4. You do **not** edit the tile internals here.
...
**Tile fidelity, by eye (the tile skill's check, repeated after the push).** With the draft shown, put one HR tile next to a native tile of the same state — the FIDELITY CHECK harness in `../tile-extractor/references/survey-snippets.md` works on the live page, or take two same-zoom crops — at desktop and 375 px, idle and hovered. Same → done. Different → classify every difference: a tile problem goes back to `tile-extractor` (never patched in `templateStyles`), a hook is mirrored onto the wrapper or the `.hr-product` cell, a theme rule that provably cannot reach the slider is restated verbatim and rescoped to the box (`references/slider-structure.md` → *Theme CSS that doesn't reach the slider*). Never a rule that re-creates the tile's look; the computed-style diff is only for finding whose rule set a differing value. Screenshot the pair into `QA/screenshots/`.
...
2. **Delegate the tile body — single entry point.** The operator invokes only this skill; *you* invoke `tile-extractor` in Step 3 to produce the `{{ TILE_BODY }}`, telling it **`target surface = recom`** (markup only, no CSS, fixed texts as `{% input %}` blocks). Never ask the operator to run the tile skill separately, and don't re-implement tile fidelity, feed mapping, price/sale/stock logic, swatches, ratings or ATC form markup here. Its FIDELITY section must be PASS or fully classified before the drop-in; carry its MISSING DATA, OPEN QUESTIONS and TEXT INPUTS into your output.
...
- [ ] `tile-extractor` invoked for the `{{ TILE_BODY }}`, **told `target surface = recom`** (markup only, no CSS, fixed texts as `{% input %}` blocks); its FIDELITY PASS or fully classified before the drop-in; MISSING DATA, OPEN QUESTIONS and TEXT INPUTS carried into the output and the hand-off. No tile logic re-implemented here.
- [ ] Rendered HR tile put **next to a native tile** after the push (desktop and 375 px) and judged by eye; every difference classified — tile (returned to `tile-extractor`), hook (mirrored onto the wrapper or `.hr-product`), or a theme rule restated verbatim and rescoped to the box — never a rule that re-creates the tile's look; `{{ CUSTOM_STYLING_BLOCK }}` empty.

...
| "The box is gone on one page" | **Placement, not data.** A selector that no longer matches after a theme change — `recoms_list` shows `selector`, `selectorMode`, `insertMode` |
...
| **Recommendation** box missing or misplaced | `recoms_list` (`selector`, `selectorMode`, `insertMode`) → `recoms_getDesign` → reproduce on the storefront |
| **Recommendation** box present but empty or not converting | `recoms_getAnalyticsForKey` (impressions at all?) → `recoms_getAnalyticsDailyForKey` (when did it stop?) → `recoms_list` |
| **Recommendation** box shows the *wrong* products | **Read the strategy — this is no longer a hand-back.** `recoms_getAlgorithm` returns the ordered steps, the global filters, the product count and `matchesBestPractice`; `recoms_list` confirms which page the box sits on and `auditLog_getEntries` whether it changed recently. The usual finding is that the box selects exactly what it was configured to select. Order of suspicion — strategy → filters → feed → design — is in `${CLAUDE_PLUGIN_ROOT}/docs/wiki/features/product-recommendations/product-recommendations.md`. Writing the fix (`recoms_updateAlgorithm`) auto-drafts the box and still needs the operator to publish |
| **Newsletter** shows the wrong products for a recipient | `newsletterContent_listCampaigns` (**once per type** — `TEMPLATE` / `AUTORESET` / `NORMAL`) → `getCampaignAlgorithm` → `previewCampaign` **twice, once as a known and once as an unknown recipient** — an algorithm that works for only one of the two is the usual defect and is invisible in the step list. Check the pinned / default / exclude lists before the steps: pinned products ignore the campaign's filters. Behaviour is in `${CLAUDE_PLUGIN_ROOT}/docs/wiki/features/newsletter-content/newsletter-content.md` |
| **Newsletter** tile looks wrong, or a campaign reports nothing | `newsletterContent_getCampaign` → `getDesign` → `renderDesign` (the real output — clipping is invisible in the source). For "the numbers are zero", check it is not an **Auto campaign configuration**, which never serves and so never has figures of its own — sum its sends with `getAnalyticsGrouped` |

...
> exposes, and the server gains tools over time — synonym, stop-word and query-rule writes arrived
> after an earlier copy of this matrix listed them as impossible, and the whole recommendation
> strategy and Retail Media surface arrived after the copy before this one. If a tool you need is not
...
| **Product data** | `productData_get`, `productData_search`, `productData_getChanges`, `productData_getFieldValues`, `productData_getReindexStatus` | Lookup by URL **or** product number. `productData_search` is the dashboard's Product Finder — free text over titles, product numbers, categories, descriptions and brands, and **the way to turn a product the customer named into the id other tools want**. It reads the search index, so a product saved but not yet re-indexed will not appear. `productData_getChanges` returns prior values **with date and source** — the fastest answer to "why did this product stop updating" |
...
| **Recommendations** | `recoms_list`, `recoms_getGeneralSettings`, `recoms_getPlacement`, `recoms_listDesigns`, `recoms_getDesign`, `recoms_getContextCrawlConfig`, `recoms_getGoogleAnalyticsSettings` | `recoms_list` carries `key`, `name`, `type` (`FRONT_PAGE`, `CATEGORY_PAGE`, `PRODUCT_PAGE`, `CART_PAGE`, `E404_PAGE`, `UPSELL_STEP`, `SEARCH_PAGE`, `OTHER`), `state`, `draft`, `designKey` and last-changed. A LIVE recommendation with a pending draft is listed **twice under the one key** — `state` tells the rows apart. Every tool resolves a key **to the draft**, so a read shows the draft and a write never touches what the shop serves. General settings, placement, crawl config and GA are **separate facet reads**. Archived and standard designs are read-only — `recoms_copyDesign` makes an editable copy |
| **Recommendation strategy** | `recoms_getAlgorithm`, `recoms_listBestPracticeAlgorithms` | **Readable — this was listed as impossible until the server gained it.** Returns the ordered steps that fill the box, the global filters every step draws through, `productCount`, `filterByGroupingKey`, `filterableFields`, and `matchesBestPractice` (the best-practice algorithm the steps still run unchanged, `null` once tuned). Steps are numbered from 1 **as the dashboard labels them** — report them by that number, it is what the customer sees. Step catalogue, filter operators and `$context` expressions: `docs://product-algorithms/format`, served by `docs_get` |
...
| **Newsletter designs** | `newsletterContent_listDesigns`, `getDesign`, `getRenderingInfo`, `renderDesign`, `listStarterTemplates`, `getStarterTemplate` | A design carries its own lifecycle state (New, Draft, Internal review, Live, Archived), but **there is no draft-of-a-LIVE-design mechanic** the way Search, Recommendations and Pages have one — so reading a design's state tells you nothing about a pending edit, because edits land in place. §2.B |
| **Newsletter campaigns** | `newsletterContent_listCampaigns`, `getCampaign`, `getCampaignAlgorithm`, `listSentCampaignsForTemplate`, `previewCampaign` | The dashboard's **Auto** (`TEMPLATE`), **Rolling** (`AUTORESET`) and **Manual** (`NORMAL`) campaigns are three separate pages there and **three separate `listCampaigns` calls** here — one `type` per call. A rolling or manual campaign carries its **own copy** of the design, so `designId` reads back as `"custom"` once the saved design moved on: normal, not an error. The per-send records an Auto campaign accumulates are **not** in `listCampaigns` — read them with `listSentCampaignsForTemplate`. `previewCampaign` shows the products a named recipient would actually get, and persists nothing |
| **Newsletter analytics** | `newsletterContent_getAnalyticsForCampaign`, `getAnalyticsDailyForCampaign`, `getAnalyticsProductsForCampaign`, `getAnalyticsGrouped`, `getAnalyticsTotals` | Figures split into identified and unidentified recipients. **An Auto campaign config never serves and so never has figures of its own** — add up its sends with `getAnalyticsGrouped` instead of reporting zero |
| **Retail Media** | `retailMedia_listCampaigns`, `retailMedia_getCampaign`, `retailMedia_getSettings`, `retailMedia_getAnalytics` | **Reachable — this area was listed as having no tools at all.** Campaign `status` is `REVIEW` / `SCHEDULED` / `ACTIVE` / `ENDED` / `CONCLUDED` / `ARCHIVED`; archived ones are a separate list (`archived=true`). `retailMedia_getSettings` gives the banner placement sizes and where campaigns may appear — **a website with no sizes configured cannot take banner campaigns at all**, which is the answer to most "why can't we book a banner" tickets. `estimatedSpend` in analytics is views × CPM, an estimate, **not a billed figure** |
...
| **Publishing is never possible — where there is something to publish** | No MCP tool promotes a draft to LIVE; that is always a human step in the dashboard, and worth saying explicitly when handing work back. **The trap is the inverse case:** Newsletter Content designs and campaigns have no draft to promote — they are created LIVE and edited in place (§2.B), so waiting for an operator to "publish" them is waiting for a step that does not exist, and the change is already out |
| **Some replacements are full, not partial** | `search_updateFilters`, `search_updateSorting`, `search_updateInitialContent`, `search_updateLinkContent`, `pages_updateDesignFilters`, `pages_updateDesignSorting`, `pages_updateConfigProductFilters`, `pages_updateConfigProductBoosts`, `recoms_updateContextCrawlConfig` and each list argument of `recoms_updateAlgorithm`, `newsletterContent_updateCampaignAlgorithm` and `newsletterContent_updateCampaignFixedProducts` replace the **entire** list. Always `get*` first and send back the complete set, or you silently disable everything you omitted. (`search_updateDesign`, `feeds_update`, `recoms_updateDesign`, `recoms_updatePlacement`, `recoms_updateGeneralSettings`, `recoms_updateGoogleAnalyticsSettings`, `newsletterContent_updateCampaign`, `pages_updateDesign` and `pages_updateConfig` *are* partial patches) |
...
| **Search config creation** | `search_createConfig`, `search_createProductEngine`, `search_setConfigProductEngine` | Target `MOBILE` / `DESKTOP` / `BOTH` / `NONE`; on `DESKTOP` / `BOTH`, `desktopDesign` `OVERLAY` / `EMBEDDED` picks the desktop design (embedded when omitted). **`BOTH` creates two configs** — mobile and desktop are separate per-device configs |
...
| **Recommendations** | `recoms_create`, `recoms_updateAlgorithm`, `recoms_applyBestPracticeAlgorithm`, `recoms_updateGeneralSettings`, `recoms_updatePlacement`, `recoms_updateSelectedDesign`, `recoms_updateDesign`, `recoms_updateContextCrawlConfig`, `recoms_updateGoogleAnalyticsSettings`, `recoms_copyDesign` | **Every one of these auto-drafts a LIVE recommendation** and leaves it DRAFT for dashboard review. **The strategy is writable — this was listed as impossible.** `recoms_updateAlgorithm` replaces the **whole** step list (§2.0) and a recommendation must keep at least one step; `recoms_applyBestPracticeAlgorithm` swaps the steps only, **keeping the shop's own global filters**. `recoms_create` makes a new recommendation from a best-practice algorithm for the page type — born DRAFT, showing nothing until published, and its `placementDivExample` is the div the shop pastes into the page. General settings cover name, type, `productCount`, `responsiveMode`, `priority`, `renderIfEmpty` and `retailMediaInjectionMode`. Design edits auto-draft **every** LIVE recommendation using that design. Placement: `selector`, `selectorMode` (`NORMAL` / `LIVE_ONCE` / `LIVE_MULTI`), `insertMode` (`REPLACE` / `PREPEND` / `APPEND` / `BEFORE` / `AFTER`); an empty-string selector resets to `#hr-recom-<key>` |
...
| **Retail Media** | `retailMedia_createProductCampaign`, `retailMedia_createBannerCampaign` | Created in **REVIEW**; publishing is a dashboard step by design — **a live campaign spends the advertiser's budget**. Every argument is a commercial decision the customer owns (which products run, at what CPM, over which dates, whether a goal stops it): ask rather than infer, and show what you are about to create before calling. Banner artwork must cover **every** placement size `retailMedia_getSettings` lists, named exactly as listed and supplied at **twice** those dimensions, as base64 image bytes — a URL is not accepted and each image is stored the moment the call runs. Neither tool generates or resizes imagery |
...
| **Newsletter designs** | `newsletterContent_createDesign`, `newsletterContent_updateDesign`, `newsletterContent_copyDesign` | **There is no draft for these.** A created or copied design is LIVE at once. An edit reaches Auto campaigns immediately and, because tile images are fetched when the recipient opens the mail, **it also changes the pictures in newsletters already sitting in inboxes** (trailing up to ~30 s). Manual and Rolling campaigns render their own copy and are unaffected until someone re-saves them in the dashboard. `copyDesign` is the safe route to change a design live campaigns use: copy it, edit the copy, repoint the campaign in the dashboard |
| **Newsletter campaigns** | `newsletterContent_createCampaign`, `updateCampaign`, `updateCampaignAlgorithm`, `updateCampaignFixedProducts` | Created **LIVE**, and the type (`TEMPLATE` / `AUTORESET` / `NORMAL`) **can never be changed** — the wrong one has to be created again. An algorithm edit reaches the campaign at once but never the newsletters already out: a manual campaign's recipients keep their products forever, a rolling campaign's until `autoresetDays` old, an Auto campaign's from the next send. **Preview before you write** — `previewCampaign` takes the same unsaved `steps` and `filters`. `includeProductIds` pins to every recipient and **ignores the campaign's filters**, so a pinned product shows even when out of stock; `defaultProductIds` reach **only** recipients Hello Retail has no behaviour on |
...
| **Running a search query** | No tool. Query `core.helloretail.com/serve/search` with the config `key` — and **only a LIVE config is queryable**, so a draft cannot be tested this way. Send it from the storefront page (`browser_evaluate` / `javascript_tool`). Replaying the overlay's own `api/v1/search/partnerSearch` request returns only the design shell or the initial content, never the query's results. A shop's `/search?q=…` page can be the platform's own results rather than Hello Retail's, so check which one renders before counting it as a reproduction |
| **Queuing a feed run ("re-sync now")** | No tool. Operator triggers it, or the customer uses the queue-run API |
| **Deleting or archiving a recommendation** | Creating one **is** possible (`recoms_create`, §2.A) and so is editing every part of an existing one. Removing one is not — operator, in the dashboard |
...
| **Retail Media: publishing, placement sizes and AI banner generation** | Campaigns are now readable and creatable (§1, §2.A). What stays in the dashboard: publishing a REVIEW campaign, configuring the placement sizes and positions, and generating banner artwork with AI. A website with no sizes configured cannot take banner campaigns until an operator sets them |
| **Uploading image assets to a newsletter design** | No tool. Reference images by absolute https URL, or `asset://` only for files the design already holds. Operator uploads in the dashboard |
| **Triggered Email designs** | No tools. The `triggered-email-developer` skill produces code for an operator to paste; QA via `newsletter-qa`. (Newsletter *Content* designs **are** covered — see §1 and §2.B) |
...
**A recommendation's `name` and `type` still do not tell you its algorithm — but you can now just
read it.** `recoms_list` returns a human label ("Top products in category - Box 1") and a page
context (`CATEGORY_PAGE`); neither is the recommendation source, and a label set when it was created
is not a read of what it runs today. What changed is that `recoms_getAlgorithm` settles it in one
call — so inferring the strategy from the name, or handing the question to the operator, is now
itself the mistake. Read `matchesBestPractice` while you are there: non-`null` means the steps are
still the stock best-practice ones nobody has tuned, which is often the finding.

**A newsletter design edit reaches mail that has already been sent.** Tile images are fetched when
the recipient opens the mail, so editing a design repoints the pictures in newsletters already
sitting in inboxes — there is no draft to hide behind (§2.B). Copy the design, edit the copy, and
let the operator repoint the campaign.

**An Auto campaign has no figures of its own.** It never serves; its sends do. Reporting zero from
`newsletterContent_getAnalyticsForCampaign` on an Auto campaign config is a misread — add up the
sends with `getAnalyticsGrouped`, or list them with `listSentCampaignsForTemplate`.
